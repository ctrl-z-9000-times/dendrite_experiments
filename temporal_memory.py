...
from connections import Connections, DendriteEvent, DendriteSegment, binSearch
...
class TemporalMemory(Serializable):
  ...
  def __init__(self,
               ...
               initialPermanence  =0.31,
               connectedPermanence=0.30,
               minThreshold=10,
    ...
    assert(initialPermanence >= connectedPermanence - EPSILON) # 
    ...
    self.iteration = 0
    self.lastUsedIterationForSegment = []   # Unused?
  ...
  def compute(self, activeColumns, learn=True):
    ...
    """
    assert(all(isinstance(c, int) for c in activeColumns))
  ...
  def activateCells(self, activeColumns, learn=True):
    ...
    for columnData in groupby2(activeColumns, identity,
                               self.activeEvents, segToCol,
                               self.matchingEvents, segToCol):
      (column,
       activeColumns,
       columnActiveEvents,
       columnMatchingEvents) = columnData
      if activeColumns is not None:
        if columnActiveEvents is not None:
          cellsToAdd = self.activatePredictedColumn(column,
                                                    columnActiveEvents,
                                                    columnMatchingEvents,
                                                    prevActiveCells,
                                                    prevWinnerCells,
                                                    learn)
  ...
  def activateDendrites(self, learn=True):
    ...
    **Pseudocode:**
    ...
    """
    activeEvents, matchingEvents = self.connections.computeActivity(
       self.activeCells,
    ...
    # Sort the segments by (cell_id, segment_id).
    self.activeEvents = sorted(activeEvents,
                               key=self.connections.segmentPositionSortKey)
    self.matchingEvents = sorted(matchingEvents,
                                 key=self.connections.segmentPositionSortKey)

    if learn:
      for event in self.activeEvents:
        for segment, start, end in event.spans:
  ...
  def activatePredictedColumn(self, column, columnActiveEvents,
                              columnMatchingEvents, prevActiveCells,
    ...
    :param columnActiveEvents: (iter) Active segment events in this column.

    :param columnMatchingEvents: (iter) Matching segments in this column.
    ...
    return self._activatePredictedColumn(
      ...
      columnActiveEvents, prevActiveCells, prevWinnerCells,
  ...
  def burstColumn(self, column, columnMatchingEvents, prevActiveCells,
                  prevWinnerCells, learn):
    """
    ...
    :param columnMatchingEvents: (iter) Matching segments in this column, or 
    ...
    return self._burstColumn(
      ...
      columnMatchingEvents, prevActiveCells, prevWinnerCells, cellsForColumn,
      self.iteration,
  ...
  def punishPredictedColumn(self, column, columnActiveEvents,
                            columnMatchingEvents, prevActiveCells,
                            prevWinnerCells):
    """
    ...
    :param columnActiveEvents: (iter) Active segments for this column, or None 
           if there aren't any.

    :param columnMatchingEvents: (iter) Matching segments for this column, or 
           None if there aren't any.
    ...
    self._punishPredictedColumn(
      self.connections, columnMatchingEvents, prevActiveCells,
  ...
  @classmethod
  def _activatePredictedColumn(cls, connections, random, columnActiveEvents,
    ...
    cellsToAdd = []
    previousCell = None
    for event in columnActiveEvents:
      if event.cell != previousCell:
        cellsToAdd.append(event.cell)
        previousCell = event.cell

      if learn:
        cls._adaptSegment(connections, event, prevActiveCells,
                          permanenceIncrement, permanenceDecrement)

        active = event.numActivePotentialSynapsesForSegment
        nGrowDesired = maxNewSynapseCount - active

        if nGrowDesired > 0:
          cls._growSynapses(connections, random, event, nGrowDesired,
  ...
  def _burstColumn(cls, connections, random, lastUsedIterationForSegment,
                   column, columnMatchingEvents, prevActiveCells,
    ...
    :param columnMatchingEvents: (iter)
    Matching segment events in this column.
    ...
    if there are any matching distal dendrite segments
      ...
      (learning)
        (optimization) if there are prev winner cells
          ...
          grow synapses to previous winner cells
    """
    if columnMatchingEvents is not None:
      numActive = lambda evt: evt.numActivePotentialSynapsesForSegment
      bestMatchingEvent = max(columnMatchingEvents, key=numActive)
      winnerCell = bestMatchingEvent.cell

      if learn:
        cls._adaptSegment(connections, bestMatchingEvent, prevActiveCells,
                          permanenceIncrement, permanenceDecrement)

        nGrowDesired = maxNewSynapseCount - numActive(bestMatchingEvent)

        if nGrowDesired > 0:
          cls._growSynapses(connections, random, bestMatchingEvent,
      ...
      if learn:
        ...
        if nGrowExact > 0:
          growth_event = cls._createSegment(connections,
                                       lastUsedIterationForSegment, winnerCell,
                                       iteration, maxSegmentsPerCell,
                                       maxSynapsesPerSegment, nGrowExact)
          cls._growSynapses(connections, random, growth_event, nGrowExact,
                            prevWinnerCells, initialPermanence,
                            maxSynapsesPerSegment)
  ...
  @classmethod
  def _punishPredictedColumn(cls, connections, columnMatchingEvents,
    ...
    :param columnMatchingEvents: (iter)
    Matching segment events for this column.
    ...
    """
    if predictedSegmentDecrement > 0.0 and columnMatchingEvents is not None:
      for event in columnMatchingEvents:
        cls._adaptSegment(connections, event, prevActiveCells,
                          -predictedSegmentDecrement, 0.0)
  ...
  def _createSegment(cls, connections, lastUsedIterationForSegment, cell,
                     iteration, maxSegmentsPerCell,
                     maxSynapsesPerSegment, min_free_synapses):
    """
    Create a segment on the connections, enforcing the maxSegmentsPerCell
    parameter.
    """
    # Get a new location in the dendrite tree.
    cell_data = connections.dataForCell(cell)
    free_spot = cell_data.find_free_segment(min_free_synapses, int(1.3 * min_free_synapses))
    if isinstance(free_spot, DendriteEvent):
        # A suitable area of the dendrite was found.
        growth_event = free_spot
    else:
        # Otherwise find-free-spots returns the parent node for a new segment.
        growth_event = DendriteEvent()
        # Enforce maxSegmentsPerCell.
        if connections.numSegments(cell) < maxSegmentsPerCell:
            # Create the segment.  Also create empty synapses on it.
            segment = connections.createSegment(cell, free_spot)
            segment._synapses.extend( [None] * min_free_synapses )
            growth_event.spans = [(segment, 0, min_free_synapses)]
        else:
            pass    # Dendrite tree is full, growth_event.spans is empty :(

    # Do TM-specific bookkeeping for the segment.
    for segment, start, end in growth_event.spans:
  ...
  @classmethod
  def _destroyMinPermanenceSynapses(cls, connections, random, event,
    ...
    """
    synapses = []
    for syn in event.synapse_iter():
        if syn is None:
            continue
        if syn.presynapticCell in excludeCells:
            continue
        synapses.append(syn)

    synapses.sort(key = lambda syn: syn.permanence)

    nDestroy = min(nDestroy, len(synapses))
    for syn in synapses[:nDestroy]:
        connections.destroySynapse(syn)
    event.num_synapses -= nDestroy
  ...
  @classmethod
  def _growSynapses(cls, connections, random, event, nDesiredNewSynapes,
                    prevWinnerCells, initialPermanence, maxSynapsesPerSegment):
    """
    Creates nDesiredNewSynapes synapses on the event's area passed in if
    ...
    for synapse in event.synapse_iter():
      if synapse is None:
        continue
    ...
    maxSynapses = event.size()
    num_free_synapses = event.num_synapses - maxSynapses
    overrun = num_free_synapses + nActual 
    if overrun > 0:
      cls._destroyMinPermanenceSynapses(connections, random, event, overrun,
                                        prevWinnerCells)

    synapse_locations = event.get_free_synapses()

    # Recalculate in case we weren't able to destroy as many synapses as needed.
    nActual = min(nActual, len(synapse_locations))

    for _ in xrange(nActual):
      i = random.getUInt32(len(candidates))
      j = random.getUInt32(len(synapse_locations))
      segment, index = synapse_locations[j]
      connections.createSynapse(segment, index, candidates[i], initialPermanence)
      # Delete from unordered lists by replacing with the last element 
      # in list and then shortening list length.  This is faster than deleting
      # an element from the middle of a list.
      candidates[i] = candidates[-1]; candidates.pop()
      synapse_locations[j] = synapse_locations[-1]; synapse_locations.pop()


  @classmethod
  def _adaptSegment(cls, connections, event, prevActiveCells,
                    permanenceIncrement, permanenceDecrement):
    """
    ...
    for segment, start, end in event.spans:
        for synapse in segment._synapses[start: end]:
            if synapse is None:
                continue
  ...
  def getPredictiveCells(self):
    ...
    previousCell = None
    predictiveCells = []
    for segment in self.activeEvents:
  ...
  def write(self, proto):
    ...
    protoActiveSegments = proto.init("activeEvents", len(self.activeEvents))
    for i, segment in enumerate(self.activeEvents):
    ...
    protoMatchingSegments = proto.init("matchingEvents",
                                       len(self.matchingEvents))
    for i, segment in enumerate(self.matchingEvents):
  ...
  def __eq__(self, other):
    ...
    if self.matchingEvents != other.matchingEvents:
      return False
    if self.activeEvents != other.activeEvents:
      return False
